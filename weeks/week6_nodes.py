...
resident_tools = [verify_resident, maintenance_ticket, search_lease]
...
RESIDENT_PROMPT = SystemMessage(content="""You are a property management assistant
...
- After verified → use search_lease tool for ANY lease policy questions.
- After verified → use maintenance_ticket tool for ANY repair or broken item.
- When user reports something broken or needing repair → call maintenance_ticket IMMEDIATELY.
  Use the verified resident's name and unit_id from state. Use their email for confirmation.
  Do NOT ask for confirmation — just submit the ticket directly.
- When answering lease questions, quote the relevant section and explain clearly.
- Do NOT say you cannot submit maintenance requests — you have the tool, use it.
...
- resident: current resident,i live here,current tenant, lives there, maintenance, repair, my unit, lease question, already a tenant
- unknown: not clear yet

The phrase "current resident" ALWAYS means resident.
...
def identify_node(state: AgentState):
    ...
    if user_type:
        # Classified — route silently to correct flow
...
def resident_node(state: AgentState):
    ...
    if verified:
        status_msg = (
            f"\n\nCURRENT STATE: VERIFIED. Name={name}, Unit={unit_id}. "
            f"Use maintenance_ticket for repairs. Use search_lease for policy questions."
        )
    else:
        attempts   = state.get("verify_attempts", 0)
        status_msg = (
            f"\n\nCURRENT STATE: NOT verified. Attempts={attempts}. "
            f"ONLY ask for unit number and email. DO NOT call any other tool until verified."
        )
